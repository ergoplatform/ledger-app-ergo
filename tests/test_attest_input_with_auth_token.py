...
from helpers.data import ADDRESS_0, AUTH_TOKEN, TX_ID
from helpers.tx_builder import TxBuilder
from helpers.unsigned_box import UnsignedBox

def test_attest_input_with_auth_token(device: Device, backend: BackendInterface, scenario_navigator: NavigateWithScenario, navigator: Navigator) -> None:
    unsigned_box: UnsignedBox = TxBuilder().input(ADDRESS_0, TX_ID, 0, 1000000000).inputs[0].box

    client = ErgoCommandSender(backend)
    
    for nb in client.attest_input(unsigned_box, AUTH_TOKEN):
        if nb != None:
            box = nb
            break
